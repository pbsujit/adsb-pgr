...
/* fetching data from shared memory using routines from 'shmsub02' */
   ...
   while ((p = p_icao(1)) != NULL) {
    if (p->lat != UNKNOWN && p->lon != UNKNOWN && strlen (p->acident) > 3) {
     ...
     state& st= plane_state [plane.flight_number];
     if (st.stat == "unknown") {
      st.stat = "level";
      st.last_alt = st.alt = p->alt;
     } else {
      st.alt = p->alt;
      double delta_alt = st.alt - st.last_alt;
      st.total_delta_alt += delta_alt;
      if (++st.ndelta >= state::MAX_DELTAS) {
        delta_alt = st.total_delta_alt / state::MAX_DELTAS;
        st.ndelta = 0;
        st.total_delta_alt = 0;
        if (delta_alt > 0) {
          st.stat = "ascending";
        } else if (delta_alt < 0) {
          st.stat = "descending";
    ...
    st.fcu_alt = p->bds.fcu_alt_40 - (int) p->bds.fcu_alt_40 % 10;
    if (st.fcu_alt < 0) st.fcu_alt = 0;
  ...
  for (int i = 0, j = planes.size (), k = 0; i < j; ++i) {
    aircraft& a = planes [i];
    info& d = plane_db [a.icao];
    state& s = plane_state [a.flight_number];
    if (d.from == "" && d.to == "" && ++d.tries < 3) {
    ...
    } else if (show == s.stat || show == "level") {
      if (s.stat == "ascending") printf ("\e[1;35m"); else if (s.stat == "descending") printf ("\e[1;31m");
      printf ("%03d %8s %6s %6s %4s %05.0f %10s to %05.0f %17s %s -> %s\n", ++k, a.flight_number.c_str(), a.icao.c_str(), d.reg.c_str(), d.type.c_str(), s.alt, s.stat.c_str(), s.fcu_alt, d.callsign.c_str(), d.from.c_str(), d.to.c_str());
    }
    printf ("\e[0;30m");
